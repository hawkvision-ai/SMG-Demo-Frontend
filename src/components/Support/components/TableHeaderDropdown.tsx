...
interface TableHeaderDropdownProps {
  label: string;
  options: string[];
  selectedValues: string[]; // These are EXCLUDED values (unchecked items)
...
const TableHeaderDropdown: React.FC<TableHeaderDropdownProps> = ({
  ...
  autoApply, onAutoApplyChange, columnKey, isDisabled = false, 
  isOpen, onToggle, sortConfig, onSort
}) => {
  ...
  // Sortable columns
  const sortableColumns = ['key', 'summary', 'status', 'category', 'priority', 'assignee', 'updated'];
  const isSortable = sortableColumns.includes(columnKey);

  // Determine dropdown position based on column
  ...
  return (
    <div className="relative dropdown-container">
      ...
      {isOpen && !isDisabled && (
        <div className={`absolute top-full ${getDropdownPosition()} mt-1 w-45 bg-white border border-gray-200 rounded-md shadow-lg z-[1000]`}>
